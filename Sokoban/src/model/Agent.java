...
//			}
	 ...
	 * @return 
	 */
	public Moves findPath(Position position1, Position position2)
	{
		nodes = new LinkedList<Agent.Node>();
		return findPath(map, position1, position2);
	}
	
	public Moves findPath(Map map, Position position1, Position position2)
	{
			// End point : we've found a goal!
...
//					}
					...
					else
					{
						...
						//for(Position position : findEmptySpacesAround(map.getPlayerPosition(), map))
						{
							...
							Moves result = findPath(map, position, position2);
							
							// If we are in a dead end : we backtrack!!
							if(result != null || (result.getMoves()).size()!=0 )
							{
								// No solution for the problem
								return result;
							}
